...
{
    Random _rand = new Random(); //создаёт объект рандомизатора, чтобы не создавать каждый раз новый
...
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы"); //Exception - выход из программы
...
void PrepareParallelMatrixMul(int[,] a, int[,] b)
{
    ...
    var threadsList = new List<Thread>(); //коллекция - улучшенный массив
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        ...
        threadsList.Add(new Thread(() => ParallelMatrixMul(a, b, startPos, endPos))); //пока 1 поток работает,запускается 2 и т.д.
        threadsList[i].Start();
    }
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
