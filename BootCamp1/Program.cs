...
/*Console.Clear(); //очистить терминал перед решением новой задачи
...
Алгоритм Бинарного поиска - ОПИСАНИЕ
...
for (int i = 0; i < array.Length; i++)
{
    for (int j = 0; j < array.Length - 1 - i; j++)
    {
        if (array[j] > array[j + 1])
            (array[j], array[j + 1]) = (array[j + 1], array[j]); //меняем элементы местами без временной перменной tmp
    }
}
Console.WriteLine($"Конечный массив: [{string.Join(", ", array)}]"); //вывод (сцепление) через указанный разделитель
*/
...
Console.Clear();
Console.Write("Введите число: ");
int n = int.Parse(Console.ReadLine()!), a0 = 0, a1 = 1, x; //мы вводили через Convert.ToInt32
for (int i = 0; i < n; i++)
{
    x = a0 + a1;
    a0 = a1;
...
/*Алгоритм Быстрой сортировки - ОПИСАНИЕ.
Смесь рекурсии и обычной сортировки
