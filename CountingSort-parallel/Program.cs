...
object lock_object = new object(); //блокировка ячейки, чтобы не было соперничества между тредами 

//сортировка подсчетом
//int[] array = {-10, -5, -9, 0, 2, 5, 1, 3, 1, 0, 1};
//int[] sortedArray = CountingSortExtended(array);
Random rand = new Random();
int[] resSerial = new int[N].Select(r => rand.Next(0, 5)).ToArray(); //создание массива в одну строку с лямбда-выражением
int[] resParallel = new int[N];

Array.Copy(resSerial, resParallel, N); //копирование массива
...
void PrepareParallelCountingSort(int[] inputArray)
{
    ...
    var threadsParall = new List<Thread>(); //создание коллекции (списка)

    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        int startPos = i * eachThreadCalc;
        int endPos = (i + 1) * eachThreadCalc;
        if (i == THREADS_NUMBER - 1) endPos = N; //последний поток
...
void CountingSortParallel(int[] inputArray, int[] counters, int offset, int startPos, int endPos)
{
    ...
    {
        lock (lock_object) //синхронизация "гонки" массивов
        {
